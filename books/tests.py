...
class BookIssueAPITest(APITestCase):
    def setUp(self):
        ...
        self.issue_detail_url = reverse('books:issue_detail', kwargs={'pk': self.issue.pk})
        self.issue_update_url = reverse('books:issue_update', kwargs={'pk': self.issue.pk})
        self.issue_delete_url = reverse('books:issue_delete', kwargs={'pk': self.issue.pk})

    def test_book_issue_str(self):
        """Проверка строкового представления BookIssue."""
        expected_str = (
            f"Взятая под запись книга: Война и мир, читатель: Иван Иванов"
        )
    ...
    def test_create_issue_book_not_available(self):
        ...
        self.book.save()
        data = {"book": self.book.id, "user": self.user.id, "due_date": "2025-12-31T00:00:00Z"}
        response = self.client.post(self.issue_create_url, data, format='json')
    ...
    def test_retrieve_issue_unauthorized(self):
        other_user = User.objects.create(email="testother@example.com",
            first_name="Сергей",
            last_name="Иванов",
            password="password123",
            phone_number="+79991234567",
